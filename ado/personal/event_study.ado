program define event_study
    ...
    * Check if normalizelead is specified; default to 1 if not
    if "`normalizelead'" == "" {
        local normalize_lead = 1
    } else {
    ...
    * Plot the results
    twoway (rarea lower_bound upper bound lags_leads, sort color(gs14) legend(off)) || ///
